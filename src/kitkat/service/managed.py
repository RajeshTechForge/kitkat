"""LLMService: provider registry and routing facade.

This is the main entry point for managed (server-side key) inference.
Register providers via :meth:`LLMService.register_provider` or use the
:func:`~kitkat.service.factory.create_llm_service` convenience constructor.

The service owns provider lifecycle — it initialises and shuts down every
registered provider. Route handlers interact exclusively through this facade
and never touch concrete provider classes directly.
"""
...
class LLMService:
    ...
    :func:`~kitkat.service.factory.create_llm_service` factory.
    """
    ...
    async def initialize(self) -> None:
        """Initialize all registered providers sequentially.

        Providers are initialized in insertion order so the first failure
        surfaces immediately without leaving later providers partially started.
    ...
    async def shutdown(self) -> None:
        ...
        Errors during individual provider shutdown are logged as warnings and
        do not prevent the remaining providers from being shut down.
        """
    ...
    async def complete(self, request: LLMRequest, provider_type: ProviderType) -> LLMResponse:
        ...
        via :meth:`~kitkat.abc.provider.LLMProvider.complete_with_retry`.
    ...
    async def stream(
        ...
        Each invocation yields one :class:`~kitkat.core.models.StreamChunk`
        per token delta. The final chunk carries ``is_final=True``, aggregated
        token usage, the finish reason, the model version, and wall-clock
        latency.
    ...
    async def health_check_all(self) -> dict[ProviderType, bool]:
        ...
        provider does not prevent the remaining providers from being checked.
    ...
    def count_tokens(self, provider_type: ProviderType, text: str) -> int:
        ...
        OpenAI / Anthropic, character-based fallback when unavailable).
    ...
    def get_capabilities(self, provider_type: ProviderType) -> ProviderCapabilities:
        ...
        Returns:
            The provider's :class:`~kitkat.core.models.ProviderCapabilities`.
    ...
    def _resolve(self, provider_type: ProviderType) -> LLMProvider:
        ...
        Raises:
            LLMProviderError: If no provider is registered for *provider_type*.
        """
